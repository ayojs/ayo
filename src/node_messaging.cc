...
ExternalSABReference ReferenceCountedSAB::ForIncomingSharedArrayBuffer(
  ...
  if (source->IsExternal()) {
    ...
    // way to serialize it.
    env->ThrowError("Can not serialize externalized SharedArrayBuffer");
    return nullptr;
  }

  SharedArrayBuffer::Contents contents = source->Externalize();
  ExternalSABReference r(new ReferenceCountedSAB(
...
Maybe<bool> ReferenceCountedSAB::AssignToSharedArrayBuffer(
    Environment* env, Local<Context> context,
...
ReferenceCountedSAB::ReferenceCountedSAB(void* data, size_t size)
  : data(data), size(size) { }

ReferenceCountedSAB::~ReferenceCountedSAB() {
  free(data);
}

MaybeLocal<SharedArrayBuffer> ReferenceCountedSAB::GetSharedArrayBuffer(
    Environment* env, Local<Context> context) {
...
MaybeLocal<Value> Message::Deserialize(Environment* env,
  ...
  if (flag_ != MESSAGE_FLAG_NONE &&
    ...
    // are handled in JS, e.g. serialized error messages.
...
void Message::AddSharedArrayBuffer(ExternalSABReference reference) {
...
class SerializerDelegate : public ValueSerializer::Delegate {
 public:
  ...
  Maybe<uint32_t> GetSharedArrayBufferId(
    ...
    ExternalSABReference reference(
        ReferenceCountedSAB::ForIncomingSharedArrayBuffer(env_,
                                                          context_,
                                                          shared_array_buffer));
    if (!reference) {
